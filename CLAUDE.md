...
>
> This knowledge base is also published as an OKF v0.1 **bundle** under
> [`knowledge/`](knowledge/) — a tree of markdown concept files, each with YAML
> frontmatter, for AI-agent and human consumption. Keep the two in sync when
> facts change.
...
up and skip inapplicable actions. Alongside the instructions file, every project
also gets a **`knowledge/` OKF (Open Knowledge Format) v0.1 bundle** — a tree of
markdown concept files (one per selected stack/database/etc.) composed from the
same section fragments (`lib/knowledge.ts`), mirroring the knowledge base for
...
| 2026-07-16 | **OKF knowledge bundle:** every scaffolded project also gets a `knowledge/` **OKF (Open Knowledge Format) v0.1** bundle (`lib/knowledge.ts`), composed from the *same* `CLAUDE.section.md` + `template.json` fragments that feed CLAUDE.md — one concept file per selected stack/database/vector-db/orm/iac/storage/auth/security/mcp, under `knowledge/<area>/<id>.md`, plus root `index.md` (okf_version), `overview.md`, `log.md`, and per-area `index.md`. Nav index.md files regenerate each run; overview/log/concepts are create-once (never clobbered on add-runs) | New projects are OKF-native by default; single source of truth = no per-stack duplication in the registry (reuses existing section fragments) |
...
- **Kinds:** project-type, stack, database, **vector-db**, **orm**, storage,
  ...
  §9.5; `security` + `mcp` added 2026-07-14 — see the notes below.)
...
src/
  ...
  lib/
    ...
    knowledge.ts      # compose the OKF knowledge/ bundle (concept files from the same section fragments)
